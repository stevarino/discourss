...
import {
  SETTINGS_TAB, FEEDS_TAB, SHEET_HEADERS, EXPECTED_HEADERS, HEADER_LOOKUP,
...
function setupFeedsTab(sheet: Sheets.Spreadsheet) {
  ...
  const rows: (string|number)[][] = range.getValues();
  while (rows.length < 2) {
    rows.push([]);
  }
  ...
  if (newData[0].length > 0) {
    ...
    rowA.setTextStyle(
      SpreadsheetApp.newTextStyle().setFontSize(16).setBold(true)
      .setForegroundColor('#ffffff').build());
    rowB.setBackground('#4285f4') // cornflower blue
    rowB.setTextStyle(
      SpreadsheetApp.newTextStyle().setFontSize(10).setBold(false)
      .setForegroundColor('#ffffff').build());
    tab.autoResizeColumns(cols+1, newData[0].length);
  }
}

function setupSettingsTab(sheet: Sheets.Spreadsheet) {
  let tab = sheet.getSheetByName(SETTINGS_TAB);
  if (tab === null) {
    tab = sheet.insertSheet(SETTINGS_TAB);
  }
  const exists = new Set<SETTINGS_FIELDS>();
  ...
  const toAdd: [string, string|number|boolean][] = [];
  for (const field of Object.keys(SETTINGS_FIELDS) as SETTINGS_FIELDS[]) {
    if (!exists.has(field)) {

...
import {
  SETTINGS_TAB, FEEDS_TAB, LOGS_TAB, Result, STATUS, Message, Embed, 
  Feed, FeedLookup, SafeFeed, DEFAULT_APP_NAME, getDefaultSettings,
  FeedSettings, SHEET_HEADERS, EXPECTED_HEADERS, HEADER_LOOKUP,
  Settings, SETTINGS_FIELDS, SettingsRecord, LOG_LEVEL, LOG_RECORD
} from './common.js';
...
import Sheets = GoogleAppsScript.Spreadsheet;

export * from './setup.js';

function getSettings(sheet: GoogleAppsScript.Spreadsheet.Spreadsheet, logs: LOG_RECORD[]): Settings {
  ...
  for (const row of settingsTab.getDataRange().getValues()) {
    const key = row[0] as SETTINGS_FIELDS;
    if (!key || !(key in SETTINGS_FIELDS)) {
      continue;
    }
    (settings as SettingsRecord)[key] = row[1];
  }
  settings.feed_pattern_re = new RegExp(settings.feed_pattern);
  return settings;
}

export function run(settings?: Settings): void {
  ...
  try {
    ...
    for (const feed of feeds) {
      ...
      console.log(`Updated row ${feed.index} ${result?.status}: ${result?.status_text}`);
    }
  } finally {
...
function getFeeds(values: string|number[][], settings: Settings) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    for (const j in headers) {
      const field = (HEADER_LOOKUP as Record<string, keyof Feed>)[headers[j]];
      if (field !== undefined) {
        (feed  as FeedLookup)[field] = values[i][j] as typeof feed[typeof field]
      }
    }
    if (!feed.feed) {
      continue;
  ...
  feeds.sort((a: SafeFeed, b: SafeFeed) => a.time - b.time)
 ...
 * Process Feed
 */
function processFeed(feed: SafeFeed, settings: Settings): Result {
...
function parseRssXml(content: string, settings: FeedSettings): Result {
  const msg: Message = {
 ...
 */
function sendDiscordMessage(message: Message, settings: FeedSettings) {
  if (!settings.webhook) {
    return;
  }
  message = {...message};
  let content_line = settings.discord ?? '';
  ...
  message.username = settings.appname ?? DEFAULT_APP_NAME;
  if (settings.avatar_url) {
    message.avatar_url = settings.avatar_url
  }
  
  const requests: GoogleAppsScript.URL_Fetch.URLFetchRequestOptions[] = [];
  if (settings.bundle) {
    requests.push({
      method: 'post',
...
function writeLogs(sheet: Sheets.Spreadsheet, logs: LOG_RECORD[]) {
  try {
    const rows: (string|number)[][] = [['epoch', 'DateTime (UTC)', 'Level', 'Message']];

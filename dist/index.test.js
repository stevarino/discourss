...
describe('index.ts run() integration tests', () => {
    test('successfully processes feeds, triggers discord webhook, and updates feed sheet row', () => {
        ...
        run(ctx);
        const payload = (_c = (_b = (_a = fetcher.requests[DISCORD_WEBHOOK]) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.req) === null || _c === void 0 ? void 0 : _c.payload;
        ...
        assert.match(feedValues[1][5], /1 successfu/);
    });
    ...
    test('Discord rate limiting during HTTP 429', () => {
        ...
        assert.match(feedValues[1][5], /^OK:.*\b2 successful\b/);
    ...
    test('aborts if delay is too long', () => {
        ...
        const feedValues = ws.getDataRange().getValues();
        assert.strictEqual(feedValues[1][4], 'guid-1'); // latest guid
        assert.match(feedValues[1][5], /^ERROR:.*Did not finish.*\b1 unprocessed\b/);
        const diff = ctx.rateLimiter.getTime() - ctx.now;
        assert.ok(diff <= 600, `should abort before timeout period: ${diff}`);
